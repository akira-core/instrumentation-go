...
|------|-------------|------------|------|
| **otel-mongo** (v1) | `github.com/Marz32onE/instrumentation-go/otel-mongo/otelmongo` | 0.4.1 | MongoDB driver v1 封裝；寫入時注入 `_oteltrace`；`ContextFromDocument` 與解碼輔助；可選 deliver span。 |
| **otel-mongo/v2** | `github.com/Marz32onE/instrumentation-go/otel-mongo/v2` | 0.4.1 | MongoDB driver v2 封裝；與 v1 行為對齊。 |
| **otel-nats** | `github.com/Marz32onE/instrumentation-go/otel-nats/otelnats` | 0.4.1 | 核心 NATS；W3C 脈絡在訊息標頭；deliver span。 |
| **otel-nats** | `github.com/Marz32onE/instrumentation-go/otel-nats/oteljetstream` | 0.4.1 | JetStream 發布／消費／fetch；deliver span。 |
| **otel-gorilla-ws** | `github.com/Marz32onE/instrumentation-go/otel-gorilla-ws` | 0.4.1 | 在 JSON 訊息本文內傳遞 trace context（信封格式）；`NewConn` / `Dial`。 |
...
依模組路徑搭配對應 **Git tag**（前綴與模組一致，例如 `otel-mongo/v0.4.1`）：
