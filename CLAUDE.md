...
- Core-NATS request/reply spans carry `messaging.message.conversation_id` (0.7.0+) = the reply inbox subject, set on the requester's send span, reply-receive span, and responder's process span. On the send span it's a late `SetAttributes` in `recordReply` (the inbox isn't observable until the reply arrives) and is omitted entirely on timeout/error. **The late write is suppressed when the request's own destination is an inbox** (0.9.1+, `startRequestSpan` returns the verdict and `requestWith*` threads it to `recordReply`): that request already carries the conversation it belongs to — the peer's advertised inbox — from span start, and overwriting it with this request's reply inbox made one attribute hold two values during the span's life. `oteljetstream` never derives it from a message's `Reply` field, which is the native `$JS.ACK.…` subject rather than a conversation ID, but **does** set it when the message's own destination is an inbox (0.9.1+) — an inbox IS the identifier of the exchange, whatever transport carried it.
- Span names follow semconv v1.39.0's `{messaging.operation.name} {destination}` (0.9.0+): `publish {subject}` (was `send {subject}`), `request {subject}` (was `{subject} request`). `{destination}` resolves as filter-then-concrete — the subscription subject, or a JetStream consumer's single-valued filter subject, otherwise the delivered/published subject. A resolved destination differing from the concrete subject sets `messaging.destination.template`, with `messaging.destination.name` always keeping the concrete subject. Shared logic lives in `otel-nats/internal/spanname`, used by both `otelnats` and `oteljetstream`.
- **Any span whose resolved destination is an UNBOUNDED reply inbox drops the destination from its name** (0.9.0+; extended to JetStream in 0.9.1) — `receive` (reply-receive), `publish` (a reply sent as `conn.Publish(msg.Reply, …)`), `process` (a handler on an inbox subscription). All carry `messaging.destination.name`, `messaging.message.conversation_id`, `messaging.destination.temporary=true` and `.anonymous=true` (`spanname.InboxAttrs`, shared by both packages); only the **name** omits the destination, because semconv scopes the low-cardinality rule to the span name and leaves `destination.name` Conditionally Required with no temporary/anonymous carve-out. The test runs on the **resolved** destination, not on `msg.Subject`: a subscription to `<inbox>.>` has a filter carrying the nuid too. `recordReply` is the one path that hardcodes the markers rather than testing a prefix — its span is structurally always an inbox, including one whose prefix this connection would not recognise.
- **"Unbounded" is load-bearing** (0.9.1+). A resolved destination that is nothing but a recognised inbox prefix plus wildcard tokens (`_INBOX.>`, `_INBOX.*.*`) is a fixed string the subscriber declared, so it **stays** in the span name and sets `messaging.destination.template`. semconv attaches the temporary/anonymous exclusion to `messaging.destination.name` — its *second* choice for `{destination}` — and not to `messaging.destination.template`, its first. A filter carrying any literal token (`_INBOX.<nuid>.>`) is per-request and still drops. `spanname.Resolve` strips the **longest** matching prefix before testing the remainder, because one recognised prefix can nest inside another. The `inbox` return value stays true either way: it drives the attributes, which describe the delivery rather than the name.
- **JetStream threads the connection's inbox prefixes too** (0.9.1+, `Conn.InboxPrefixes()`). All five resolve sites — `tracedJSImpl.PublishMsg`, `tracedConsumer.Next`, `dynamicConsumeHandler`, `tracedMessagesContext.Next`, `receiveSpanner.wrap` — previously passed `nil` on the reasoning that a stream never captures inbox subjects. NATS does not enforce that: a stream over `_INBOX.>` is legal and is how request/reply-over-JetStream makes replies durable, and an unfiltered consumer over one resolved to the concrete per-request subject. The four consume sites share `resolveMsgSpan` (`oteljetstream/consumer.go`) so the inbox test, the template attribute and the markers cannot drift apart; adding a sixth consume path means calling it, not re-deriving it.
- **Inbox prefixes: two are recognised** (`inboxPrefixes` in `conn.go`) — `nc.Opts.InboxPrefix + "."` when set, and `nats.InboxPrefix` (`_INBOX.`) always. Recognising only the local one fails exactly where custom prefixes are deployed: a responder sees the **requester's** inbox in `msg.Reply`, and the requester is the side that customises, because granting it `subscribe: _INBOX.>` hands it every other client's replies while a responder needs no inbox permission (`allow_responses` covers it). Two peers on two different custom prefixes is the accepted residual gap. `oteljetstream` passes `nil` prefixes — streams do not capture inbox subjects.
- **No subject-template option, deliberately.** Subjects embedding IDs (`orders.12345.created`) keep the concrete subject in the span name on publish/request paths and on JetStream consumers with no filter or several wildcard filters. semconv permits recording a `messaging.destination.template` that is already known — here a subscription or filter subject — and says nothing that licenses *inferring* one. A `WithSubjectTemplate(func(string) string)` option was built and removed before release: it contributed nothing to the inbox problem (a `string` return cannot express "omit the destination"), and the residual ID case is better handled by the collector's `span` processor `name.to_attributes` rules. Re-adding an option later is non-breaking; shipping one is not.
