...
- **Any core-NATS span whose resolved destination is a reply inbox drops the destination from its name** (0.9.0+) — `receive` (reply-receive), `publish` (a reply sent as `conn.Publish(msg.Reply, …)`), `process` (a handler on an inbox subscription). All three carry `messaging.destination.name`, `messaging.message.conversation_id`, `messaging.destination.temporary=true` and `.anonymous=true`; only the **name** omits the destination, because semconv scopes the low-cardinality rule to the span name and leaves `destination.name` Conditionally Required with no temporary/anonymous carve-out. The test runs on the **resolved** destination, not on `msg.Subject`: a subscription to `<inbox>.>` has a filter carrying the nuid too. `recordReply` is the one path that hardcodes the markers rather than testing a prefix — its span is structurally always an inbox, including one whose prefix this connection would not recognise.
...
- **No subject-template option, deliberately.** Subjects embedding IDs (`orders.12345.created`) keep the concrete subject in the span name on publish/request paths and on JetStream consumers with no filter or several wildcard filters. semconv forbids instrumentation from *inferring* `messaging.destination.template` — only from recording one already known, which here means a subscription or filter subject. A `WithSubjectTemplate(func(string) string)` option was built and removed before release: it contributed nothing to the inbox problem (a `string` return cannot express "omit the destination"), and the residual ID case is better handled by the collector's `span` processor `name.to_attributes` rules. Re-adding an option later is non-breaking; shipping one is not.
