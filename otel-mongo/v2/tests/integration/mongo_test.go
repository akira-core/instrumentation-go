...
// in the document and the stored TraceID matches the insert span's TraceID.
func TestIntegration_InsertOneInjectsOtelTrace(t *testing.T) {
	...
	coll := client.Database("integ_v2").Collection("insert_one")
...
func TestIntegration_InsertManyAllHaveOtelTrace(t *testing.T) {
	...
	coll := client.Database("integ_v2").Collection("insert_many")
	t.Cleanup(func() { _ = coll.Drop(context.Background()) })
...
func TestIntegration_CursorDecodeAndTraceExtractsTrace(t *testing.T) {
	...
	coll := client.Database("integ_v2").Collection("cursor_decode")
	t.Cleanup(func() { _ = coll.Drop(context.Background()) })
...
func TestIntegration_SingleResultTraceContextExtractsTrace(t *testing.T) {
	...
	coll := client.Database("integ_v2").Collection("single_result")
	t.Cleanup(func() { _ = coll.Drop(context.Background()) })
...
// TestIntegration_UpdateOneInjectsTrace verifies that UpdateOne stores _oteltrace
// from the update span's context in the document.
func TestIntegration_UpdateOneInjectsTrace(t *testing.T) {
	tp, _ := newTestProvider()
	setupOtel(tp)
	...
	coll := client.Database("integ_v2").Collection("update_trace")
	t.Cleanup(func() { _ = coll.Drop(context.Background()) })
