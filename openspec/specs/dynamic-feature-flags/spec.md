# dynamic-feature-flags Specification

## Purpose
Runtime control of the instrumentation switches through OpenFeature: who owns the
provider, how a GO Feature Flag relay proxy is reached, and what a process resolves
while the relay is silent, unreachable, or not yet ready.
...
1. `true` when the process installed a provider through `SetNamedProvider` (see *Application-installed providers*); otherwise
...
Step 2 admits exactly one false negative — an application that binds the same provider *name* to both the default slot and `FlagDomain` reads as unbound. `SetNamedProvider` SHALL close it for any application that cares. Only the public OpenFeature API SHALL be used: `GetNamedProviders()` would answer exactly but returns the live map without copying, so reading it races a concurrent `SetNamedProvider` and can crash the process with `concurrent map read and map write`.
...
- **WHEN** an application binds the same provider type to the default slot and installs one through `SetNamedProvider`
...
An application that requires the relay's answer before its first operation SHALL install its own provider with `SetNamedProvider` before constructing any wrapper, which both closes the window and makes the auto-install path stand down. This SHALL be documented as the application's option, not as a requirement on it.
...
- **WHEN** an application calls `SetNamedProvider` before constructing any wrapper
...
`otel-flags` SHALL NOT contain module-scoped OpenFeature flag keys, module-scoped environment variable names, or module-scoped hardcoded defaults. `Resolver` SHALL accept the OpenFeature flag key as a **parameter of `Value`**, supplied by each module's own `env_flags.go`, which also owns the paired environment variable and the tier's default and passes the resolved `local` value into the same call. The resolver SHALL NOT hold a per-resolver key list, because an index into one couples two modules' flags by position with nothing checking it, and SHALL NOT hold or read any module-scoped environment variable name.
...
| — | `RelayPossible() bool`, `ValidateAndInstall() error`, `SetNamedProvider(p) error`, `Lookup(name) (bool, bool, error)`, `ErrInvalidFlagValue` |
