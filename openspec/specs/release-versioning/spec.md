...
Each of the five **released** modules (`otel-nats`, `otel-mongo`, `otel-mongo/v2`, `otel-gorilla-ws`, `otel-sampler`) SHALL carry a `CHANGELOG.md` inside its module directory, so the file is included in the Go module zip served by the module proxy. The file SHALL follow the Keep a Changelog structure (one section per released version, newest first). The four instrumentation modules' CHANGELOGs SHALL be backfilled from `0.6.0` onward with a pointer line noting that pre-0.6.0 history was released from the legacy branch line and is not covered; `otel-sampler`'s SHALL start at its first entry. Every release tag SHALL be preceded by a CHANGELOG entry for that version in the same module.

`otel-testkit` is a test-only harness and SHALL NOT be tagged or released; it therefore carries no CHANGELOG and no version constant.
...
#### Scenario: Supporting module release status is unambiguous
- **WHEN** a contributor asks whether `otel-sampler` or `otel-testkit` may be tagged
- **THEN** the documentation states that `otel-sampler` is released under `otel-sampler/vX.Y.Z` while `otel-testkit` is untagged and test-only

### Requirement: Written versioning policy

The repository SHALL contain a root-level `VERSIONING.md` documenting: the tag format `<module>/v<x.y.z>` and its `otel-mongo/v2` exception (below); that each module versions independently; the pre-1.0 policy that breaking changes require at least a **minor** bump while additive features and fixes may use a patch bump; where release notes live (module `CHANGELOG.md` plus GitHub Releases); and the source location of each released module's version constant (`otel-nats/otelnats/conn.go`, `otel-mongo/otelmongo/version.go`, `otel-mongo/v2/version.go`, `otel-gorilla-ws/version.go`, `otel-sampler/otelsampler/version.go`).

`VERSIONING.md` SHALL distinguish the four instrumentation wrappers — whose constant is additionally reported as `otel.scope.version` on every span they emit — from `otel-sampler`, which emits no spans and whose constant serves only the release guard and callers recording their sampler build.
...
`VERSIONING.md` SHALL record that the published `otel-sampler/v0.1.0` tag points at a commit that is not an ancestor of `main` and resolves to content not matching the repository. Because the Go module proxy caches versions immutably, that number SHALL NOT be re-cut; `otel-sampler` releases start at `0.1.1`. `VERSIONING.md` SHALL further record that `0.1.1` is a patch bump despite carrying behavioral changes, scoped as a one-off justified by `0.1.0` never having resolved to matching content, and that subsequent `otel-sampler` releases follow the normal pre-1.0 rule.
...
#### Scenario: Downstream avoids the superseded sampler tag
- **WHEN** a consumer looks up which `otel-sampler` version to depend on
- **THEN** `VERSIONING.md` states that `v0.1.0` is superseded and unusable, and that `0.1.1` is the first version whose content matches the repository

### Requirement: Release-tag version guard in CI

A CI workflow SHALL trigger on pushed tags matching the five module tag patterns (`otel-mongo/v[0-9]*`, `otel-mongo/v2/v[0-9]*`, `otel-nats/v[0-9]*`, `otel-gorilla-ws/v[0-9]*`, `otel-sampler/v[0-9]*` — a single `otel-*/v*` glob cannot match tags containing a second `/` because GitHub Actions tag globs do not cross `/`), parse the module path and semantic version from the tag, extract that module's version constant from its documented source location, and fail when the two values differ. The guard SHALL print both values in its failure output.
...
`otel-sampler/v*` tags SHALL validate against `otel-sampler/otelsampler/version.go` using the same `^\s*(const\s+)?instrumentationVersion\s*=` extraction the other constant-bearing modules use.
...
#### Scenario: Sampler tag is guarded like the wrappers
- **WHEN** the tag `otel-sampler/v0.1.1` is pushed
- **THEN** the guard validates it against `instrumentationVersion` in `otel-sampler/otelsampler/version.go` and fails if the two differ
