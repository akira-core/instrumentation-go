# span-link-sampling-seed Specification

## Purpose
Sampling-input adaptation for root spans that carry span links: `WithSingleLinkSeed` seeds randomness from exactly one valid link so async span-link consumers (NATS subscribers, MongoDB change-stream readers, WebSocket readers) sample consistently with their producer, without altering the parentage or links the SDK creates.
