...
## Purpose
Application-side consistent probabilistic sampling: a threshold-based `ProbabilitySampler` aligned with OpenTelemetry Go PR #8123, reading and writing the `ot=th:` / `ot=rv:` tracestate sub-keys so services at different rates make coherent sample/drop decisions for the same trace.
