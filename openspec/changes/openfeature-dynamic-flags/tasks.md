...
- [x] 9.4.2 Gate negotiation on the static capability from 9.3.3 in `Dial` and `Upgrader.Upgrade`; confirm `TestUpgrader_TracingDisabled_DoesNotNegotiateOTelWS` and `TestDial_TracingDisabled_DoesNotOfferOTelWS` still pass and add the module-switch-off case.
- [x] 9.4.3 Update `otel-ws.md` §5: capability is now fully static and there is no relay-driven negotiation exception. Add a section declaring `{"header":…,"data":…}` a **reserved** wire structure on an `otel-ws` connection — an application payload of that shape is unwrapped and its outer structure discarded. The document currently does not say so.
...
- [x] 9.5.1 Rewrite every test that sets a tracing env var **and** passes the matching option (~89 call sites across 11 files) to use exactly one of them.
- [x] 9.5.1b Rewrite any test that changes a **module** environment variable after constructing a wrapper and expects the change to take effect. Under D7 the module env var is read once, at construction, so such a test must now reconstruct the wrapper. Only the relay verdict is observable without reconstruction — tests exercising a live change should mutate the in-memory provider instead.
- [x] 9.5.2 Add per-module kill-switch asymmetry tests: relay `true` + module env off ⇒ no spans and no evaluation; relay `false` + module env on ⇒ the running connection's next operation emits no span.
- [x] 9.5.3 Add constructor-conflict tests for all seven option-accepting constructors, asserting `errors.Is` against the module sentinel.
- [x] 9.5.4 Update the relay integration test to assert the revoke direction (start enabled, revoke, observe stop) instead of enabling from off.
- [x] 9.5.6 `otel-gorilla-ws`: a conn that proved `otel-ws` with capability off returns the **unwrapped** payload from `ReadMessage` and still writes raw (9.4.5); a JSON-object payload carrying neither trace key round-trips byte-identically, key order included (9.4.6); `SubprotocolOTelWS`/`IsOTelNegotiated` agree with what `NewConn` actually does (9.4.4).
- [x] 9.5.7 Confirm no test in any module leaves `OTEL_INSTRUMENTATION_GO_FLAGS_ENDPOINT` set, and that every in-memory provider is installed with `SetNamedProviderAndWait(FlagDomain, …)`.
- [x] 9.5.5 Run `go build`, `go test -race`, `golangci-lint` in every touched module until clean.
...
- [x] 9.7.1 Correct `CLAUDE.md`'s transport table: `_oteltrace` is **not** stripped on read. Check `README`s and module docs for the same claim.
