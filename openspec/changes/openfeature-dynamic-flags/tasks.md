...
- [ ] 9.8.1 Update `feature-flags.md` end to end for the model this change lands: the zero-code wiring path and the three environment variables, the auto-install trigger and what makes it stand down, revocation latency as the provider's poll interval, the two limits an operator would otherwise misread (extraction is not stopped by a revocation; the WebSocket envelope survives one), and the `service.name` targeting attribute. Supersedes the status banner at the top of the file.
- [ ] 9.8.2 Add `feature-flags.zh-TW.md`, a Traditional Chinese translation kept in step with `feature-flags.md`, following the existing `README.md` / `README.zh-TW.md` convention. Cross-link the two at the top of each.
