...
- **Source unchanged in shape**: `parseServerFromURI` and `Client.serverAddr`/`serverPort` remain as the fallback path (v1's `lastNonEmptyURI(opts)` was retired during implementation; the fallback URI derives from `parseServerFromURI(merged.GetURI())`, matching v2). `shared.DBAttributes`' signature **does** change (drops `serverAddr string, serverPort int`) — an internal-only helper, no public API impact; its `server.*` tail moves verbatim into the new `shared.ServerAttributes(addr, port)`.
