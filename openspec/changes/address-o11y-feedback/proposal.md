...
- **API**: additive options everywhere (R1), with one signature change: `Upgrader.Upgrade` gains a variadic `opts ...Option` parameter — source-compatible for ordinary 3-arg calls, but a Go-type break for method values / interface assignments pinned to the old signature (labeled **Changed — BREAKING** in `otel-gorilla-ws/CHANGELOG.md` with a closure-wrap migration note). Attribute key change (F2) and span-duration change (R5) are behavioral breaks → **otel-nats 0.7.0**; other modules get their own minor bump when R1 lands in them.
