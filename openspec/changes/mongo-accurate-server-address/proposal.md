...
- Thread the captured address through each Collection CRUD operation in `internal/traced/collection.go` via a per-call `context.Context`-scoped holder. Static `db.*` attributes (including a best-effort static `server.address`) stay attached at span start as today; only `server.address`/`server.port` are overwritten after the raw driver call via `span.SetAttributes(shared.ServerAttributes(addr, port)...)` — an additive one-block change per site, not a restructure. `shared.DBAttributes`' signature is unchanged.
...
- **Source added**: a `CommandMonitor` constructor + `ConnectionID` parser + context-scoped address holder in `internal/shared/` (new file), a `ServerAttributes` helper split out of `DBAttributes` in `internal/shared/semconv.go` (`DBAttributes` calls it internally so the two paths can't drift), `MergeClientOptions`-based monitor registration in `client.go` (`ConnectWithOptions`), and a one-block post-call `span.SetAttributes(shared.ServerAttributes(...)...)` addition at every `shared.DBAttributes(...)` call site in `internal/traced/collection.go` (14 sites in v1, 16 in v2).
- **Source unchanged in shape**: `parseServerFromURI`, `lastNonEmptyURI`, `Client.serverAddr`/`serverPort` remain as the fallback path; `shared.DBAttributes` signature is unchanged (still takes `serverAddr string, serverPort int`, just sourced differently by the caller).
