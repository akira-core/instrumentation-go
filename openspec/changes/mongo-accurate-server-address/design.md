...
### 2. Correlation: context-scoped mutable holder; overwrite server.* post-call, keep static db.* at span start
Each traced Collection method already creates its span before calling the raw driver method and passes `ctx` through. Immediately after `tracer.Start(...)` we stash a pointer to a small holder (`*addrCapture{addr string, port int}`) into that `ctx` via `context.WithValue` (`shared.WithAddrCapture(ctx)`), then pass the holder-`ctx` to the raw `t.Coll.XxxOne(ctx, ...)` call. `CommandMonitor.Started` runs synchronously in the same goroutine with a ctx derived from ours, reads the holder key, and writes the parsed `ConnectionID` into it. When the raw call returns, the traced method reads the holder (`capture.Resolve(t.ServerAddr, t.ServerPort)`, falling back to the static value when nothing was captured) and calls `span.SetAttributes(shared.ServerAttributes(addr, port)...)`.
  ...
  This is **additive to each call site, not a restructure**. The static `db.*` attributes — including a best-effort static `server.address`/`server.port` from `t.ServerAddr`/`t.ServerPort` — stay attached at `tracer.Start(..., trace.WithAttributes(shared.DBAttributes(...)...))` exactly as today. We only *overwrite* `server.address`/`server.port` afterward with the per-command value. `DBAttributes`' signature is unchanged; each site gains one post-call block:
  ```go
  ctx, span := t.Tracer.Start(ctx, name, trace.WithSpanKind(trace.SpanKindClient),
      trace.WithAttributes(shared.DBAttributes(dbName, collName, op, batchSize, t.ServerAddr, t.ServerPort)...))
  defer span.End()
  ...
  if addr, port := capture.Resolve(t.ServerAddr, t.ServerPort); addr != "" {
      span.SetAttributes(shared.ServerAttributes(addr, port)...)   // NEW: overwrite server.*
  }
  shared.RecordSpanError(span, err)                    // unchanged
  ```

  **Why overwrite-after, not move all attributes post-call.** The OTel Go SDK builds the exported snapshot at `span.End()`, *not* at `tracer.Start()`: any `SetAttributes` before `End()` is fully reflected in exported data, and a later `SetAttributes` with the same key replaces the earlier value. There is no "start-time attributes are permanently snapshotted" behavior to design around — so overwriting `server.address`/`server.port` post-call yields exactly the per-command value in the export. Keeping the other `db.*` attributes at start (rather than moving the whole `DBAttributes` list to a post-call `SetAttributes`) has two concrete advantages:
  - **Robustness**: if the raw call panics or returns early and `span.End()` fires via `defer`, the span still carries `db.system.name`/`db.operation.name`/`db.namespace` (and best-effort static `server.address`) rather than being attribute-less.
  - **Sampling**: samplers that inspect start-time attributes still see the full `db.*` set.

  Only cost: `server.address`/`server.port` are set twice when the captured value differs from the static one (a same-key replace on a live, not-yet-ended span) — negligible. `internal/direct` is untouched; only `internal/traced/collection.go` gains the post-call block.

  **Helper split.** Factor the `server.address`/`server.port` tail of `DBAttributes` (`semconv.go`, the `if serverAddr != "" { ... }` block) into `shared.ServerAttributes(addr string, port int) []attribute.KeyValue`, and have `DBAttributes` call it internally so the start-time and post-call paths cannot drift. `ServerAttributes` returns `nil` for an empty address and omits `server.port` for the default 27017 — identical rules to today.
...
- **[Trade-off] Default-port omission interacts with the capture.** `ServerAttributes`/`DBAttributes` omit `server.port` when the port is the default 27017 (unchanged rule). So a command served by a host on 27017 emits `server.address` only, no `server.port` — even after per-command capture. Test assertions for the failover/replica-set scenarios must account for this (a captured `host:27017[-1]` yields `server.address` alone).
